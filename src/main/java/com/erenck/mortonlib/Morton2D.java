...
public class Morton2D {
	...
	private static int decodeHelper(long c, int[] coord) {
		...
		for (long i = 0; i < loops; ++i) {
			a |= ((long) coord[(int) ((c >> (i * 8)) & EIGHTBITMASK)] << (4 * i));
		}
		return (int) a;
	}
	...
	public long encode(int x, int y) {
		...
		return ((long) MortonTable256[(y >> 8) & EIGHTBITMASK] << 17
                | (long) MortonTable256[(x >> 8) & EIGHTBITMASK] << 16
                | (long) MortonTable256[y & EIGHTBITMASK] << 1
